...
function [corr_times, range_starts, range_ends] = associator_plot(rec_dict_tseries, ...
    ...
    if ~isempty(sig_intervals(1,:))
        ...
        for i = 1:length(sig_intervals(1, :))
            ...
            num_receivers = length(rec_dict_tseries);
            ...
            for j = 2 :length(rec_dict_tseries(:,1))   % Start at 2 bc 1 is always reference   
                ...
                if corr_type == 1 % 1D, time series correlation
                    ...
                    subplot(4, 3, 3*j-5);
                    plot(linspace(ref_duration(1), ref_duration(2), length(ref_tseries_filt)), ref_tseries_filt);
                    if j == 2
                        title('Reference Signal Time Series', 'FontSize', 20);
                    end
                    if j == 5
                        xlabel('Seconds Elapsed', 'FontSize', 20);
                    end
                    ylabel("Receiver " + num2str(j), 'FontSize', 20);
                    
                    subplot(4, 3, 3*j-4);
                    plot(linspace(j_duration(1), j_duration(2), length(j_tseries_filt)), j_tseries_filt);
                    if j == 2
                        title('Other Receiver Time Series', 'FontSize', 20);
                    end
                    if j == 5
                        xlabel('Seconds Elapsed', 'FontSize', 20);
                    end
                    ...
                    subplot(4, 3, 3*j-3);
                    plot(j_duration(1) + lags / samp_rate, r);
                    if j == 2
                        title('Cross-Correlation', 'FontSize', 20);
                    end
                     if j == 5
                        xlabel('Seconds Elapsed', 'FontSize', 20);
                    end
